...
class ShareInfoController extends ApiController {
	 ...
	 * @return JSONResponse<FilesSharingShareInfo, Http::STATUS_OK>|JSONResponse<\stdClass::class, Http::STATUS_FORBIDDEN|Http::STATUS_NOT_FOUND>
	 *
	 * 200: Share info returned
	 * 403: Getting share info not allowed
	 * 404: Share not found
	 */
	public function info(string $t, ?string $password = null, ?string $dir = null, int $depth = -1): JSONResponse {
		try {
			...
			$response = new JSONResponse(\stdClass::class, Http::STATUS_NOT_FOUND);
			$response->throttle(['token' => $t]);
			return $response;
		}

		if ($share->getPassword() && !$this->shareManager->checkPassword($share, $password)) {
			$response = new JSONResponse(\stdClass::class, Http::STATUS_FORBIDDEN);
			$response->throttle(['token' => $t]);
			return $response;
		}
